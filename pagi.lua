...
SlidersFrame.Size = UDim2.new(1, 0, 0, 170)
...
TogglesFrame.Position = UDim2.new(0, 0, 0, 170)
...
local function UpdateLayout()
    if settingsCollapsed then
        Tween(SettingsFrame, {Size = UDim2.new(1, 0, 0, 170), Position = UDim2.new(0, 0, 1, -170)})
        Tween(ScrollList, {Size = UDim2.new(1, -10, 1, -275)})
        TogglesFrame.Visible = false
    else
        Tween(SettingsFrame, {Size = UDim2.new(1, 0, 0, 510), Position = UDim2.new(0, 0, 1, -510)})
...
AutoBlatantLabel.Text = string.format("Auto Blatant Timer: %ds", autoBlatantTimer)
...
elseif blatantMode == "Auto" then
    BlatantBtn.TextColor3 = Color3.fromRGB(255, 180, 80)
else
...
runConn = RunService.RenderStepped:Connect(function()
    local success, err = pcall(function()
        ...
        if blatantMode == "Auto" and seconds then
            isBlatant = seconds <= autoBlatantTimer
        else
            isBlatant = blatantMode == "On"
        end
