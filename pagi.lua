...
-- Precomputed Killer Suffix Set (only suffixes <= 5 chars)
local KillerSuffixSet = {}
do
    local rawSuffixes = {
    ...
    for _, suf in ipairs(rawSuffixes) do
        if #suf <= 5 then
            KillerSuffixSet[suf] = true
        end
    end
end
...
-- Optimized EndsWithKillerSuffix using hash lookup and max 5-char check
local function EndsWithKillerSuffix(word)
    local len = #word
    for checkLen = 1, math.min(5, len) do
        local suf = word:sub(-checkLen)
        if KillerSuffixSet[suf] then
            return true
        end
    end
    return false
...
RandomizeTopBtn.Position = UDim2.new(0, 15, 0, 142)
...
ManageWordsBtn.Position = UDim2.new(0, 150, 0, 142)
...
local limit = 100
local maxProcess = 300 -- HARD LIMIT FOR PERFORMANCE
if bucket then
...
if #exacts >= maxProcess then return true end -- EARLY BREAK
...
-- Optimized RandomizeTop without repeated insert/remove
if (sortMode == "Longest" or sortMode == "Shortest" or sortMode == "Killer") and randomizeTop and #matches > 1 then
    ...
    for i = 1, #matches do
        if matches[i] then
            compact[idx] = matches[i]
            idx += 1
        end
    end
    -- Shuffle top
    shuffleTable(topPart)
    -- Rebuild matches: shuffled top + rest
    matches = {}
    for i = 1, #topPart do
        table.insert(matches, topPart[i])
    end
    for i = 1, #compact do
        table.insert(matches, compact[i])
    end
end
end
local displayList = {}
