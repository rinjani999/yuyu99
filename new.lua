...
-- === TOMBOL-TOMBOL UI (Didefinisikan dulu) ===
...
WBHeader.Size = UDim2.new(1, 0, 0, 35)
WBHeader.BackgroundTransparency = 1

local WBCloseBtn = Instance.new("TextButton", WordBrowserFrame)
WBCloseBtn.Text = "X"
WBCloseBtn.Font = Enum.Font.GothamBold
WBCloseBtn.TextSize = 14
WBCloseBtn.TextColor3 = Color3.fromRGB(255, 100, 100)
WBCloseBtn.Size = UDim2.new(0, 30, 0, 30)
WBCloseBtn.Position = UDim2.new(1, -30, 0, 2)
WBCloseBtn.BackgroundTransparency = 1
WBCloseBtn.MouseButton1Click:Connect(function() WordBrowserFrame.Visible = false end)

local WBSearchBox = Instance.new("TextBox", WordBrowserFrame)
WBSearchBox.Font = Enum.Font.Gotham
WBSearchBox.TextSize = 12
WBSearchBox.BackgroundColor3 = THEME.ItemBG
WBSearchBox.Size = UDim2.new(1, -20, 0, 24)
WBSearchBox.Position = UDim2.new(0, 10, 0, 35)
WBSearchBox.TextColor3 = THEME.Text
Instance.new("UICorner", WBSearchBox).CornerRadius = UDim.new(0, 4)
SetupPhantomBox(WBSearchBox, "Search database...")

local WBScroll = Instance.new("ScrollingFrame", WordBrowserFrame)
WBScroll.Size = UDim2.new(1, -10, 1, -70)
WBScroll.Position = UDim2.new(0, 5, 0, 65)
WBScroll.BackgroundTransparency = 1
WBScroll.ScrollBarThickness = 2
WBScroll.ScrollBarImageColor3 = Color3.fromRGB(200, 150, 255)
WBScroll.CanvasSize = UDim2.new(0,0,0,0)

local WBListLayout = Instance.new("UIListLayout", WBScroll)
WBListLayout.SortOrder = Enum.SortOrder.LayoutOrder
WBListLayout.Padding = UDim.new(0, 2)

local function RefreshWordBrowser()
    for _, c in ipairs(WBScroll:GetChildren()) do
        if c:IsA("Frame") then c:Destroy() end
    end
    
    local queryRaw = WBSearchBox.Text
    local query = (queryRaw == "Search database...") and "" or queryRaw:lower():gsub("[%s%c]+", "")
    ...
    if Words then
        for _, w in ipairs(Words) do
            if query == "" or w:find(query, 1, true) then
                ...
                local row = Instance.new("Frame", WBScroll)
                ...
                if shownCount >= limit then break end
            end
        end
    end
    WBScroll.CanvasSize = UDim2.new(0, 0, 0, shownCount * 24)
end
WBSearchBox:GetPropertyChangedSignal("Text"):Connect(RefreshWordBrowser)

-- 3. Blacklist Manager Window
...
-- === LOGIKA WINDOW & TOMBOL (Dihubungkan di akhir agar variabel sudah ada) ===

-- Helper Func: Refresh List (untuk Custom Words)
...
WordBrowserBtn.MouseButton1Click:Connect(function()
    ...
    if WordBrowserFrame.Visible then RefreshWordBrowser() end
end)
